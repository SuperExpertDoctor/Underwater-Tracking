...
## 任务 3：接入真实 Embedding 和结构化记忆 LLM
...
## 任务 4：实现 MemoryService 和持久化后台 MemoryWorker
...
## 任务 5：接入 Conversation、CarrierRuntime 和证据回溯
...
## 任务 6：暴露真实 FastAPI 记忆接口并接入运行时生命周期
...
## 任务 8：增加底部 Memory Steam 并保留独立 LLM 思考流
...
## 任务 9：实现 60 倍仿真调度和 1x/4x/10x 视频回放
