...
固定种子场景必须验证：至少两艘母舰；同一母舰连续访问至少三个投放点；从航母战斗群出发并最终返回；路线不穿过区域；投放前 ACTIVE_SCAN；目标进入后 PASSIVE_TRACK；目标离开后交接与回收；最大里程触发轮换和重规划；意图/置信度变化只有在连续确认且影响当前方案时重新生成区域；正常投放/回收不触发战略 LLM；资源不足不全派；新帧无 USV；同 seed 与真实 LLM provider 产生可校验计划和路线。
...
第一阶段只有在以下条件全部满足时完成：UUV 完成主动扫描与被动跟踪切换；区域权严格交接；每个目标最终执行区域不超过 4 个且候选证据完整；里程事件触发回收和重规划；普通事件不触发战略 LLM；关键事件只在方案质量/效能/可行性受影响时触发；事件 episode 有进入/恢复滞回；所有事件、观测和人机交互进入记忆处理；LLM 只能使用短期上下文完成推理；投放按整体效能分批；母舰可连续投放且有可用 UUV 时保持可匹配；每条母舰路线从航母战斗群出发并返回；A* 不穿越区域；匈牙利考虑距离、时间窗、容量和未来资源；地图显示概率栅格和黄色透明区域；实时方案按计划版本增量同步；WebSocket 断连无未处理 ASGI 异常；旧回放兼容且新帧无 USV；后端、API、前端、真实 LLM 和本地 Embedding 长时测试通过。
