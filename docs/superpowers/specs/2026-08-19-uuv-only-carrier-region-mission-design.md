...
预测网格和候选区域全集继续保留，用于证据回溯、审计和后续重规划；最终执行方案按目标最多保留 4 个任务区域。该上限在任务优化和最终任务物化边界强制执行，适用于 UUV-only 和兼容的区域任务视图。

区域选择必须稳定且可解释：当前正在执行的区域及其连续后继优先，再按预测概率、任务优先级、时间窗和代价补足。超出上限的候选不删除，标记为 `region_cap_not_selected`，不能出现在执行任务集合中，也不能被伪装成已覆盖区域。

LLM 请求的“每批最多 4 个区域/候选区域”只是输出预算约束，与最终每个目标最多 4 个执行区域是两个独立规则。

### 2.5 固定航母战斗群编成

UUV-only 环境初始化时固定生成一个航母战斗群：1 艘航母和 3 艘母舰。四艘舰从配置给出的固定初始位置开始航行，并在运行时保持稳定的角色标识：

- `carrier_01` 是航母，只作为编队主节点和母舰返航的航母战斗群归属点，不承接 UUV 投放/回收批次；
- `carrier_02`、`carrier_03`、`carrier_04` 是母舰，分别维护自己的 UUV 库存，执行投放和回收；
- UUV 初始归属只分配给母舰，航母不应出现在 UUV 后勤归属映射中；
- UUV 的后勤归属在初始化后保持不变；新计划如果把既有 UUV 的批次或库存改派到另一艘母舰，必须在物理执行边界拒绝，任务轮转不能通过改写归属实现；
- 每艘母舰的投放/回收任务必须从自己的初始编队位置出发，完成停靠任务后回到同一编队位置；
- 物理数据和操作帧同时保留单一主航母兼容字段与完整的四舰列表，不能把所有 UUV 关系错误汇总到航母字段。
...
普通模式下，区域内所有已编入任务的 UUV 使用分配到自身的蛇形覆盖航迹；具备主动能力的 UUV 将主动声呐目标绑定到当前区域的目标估计，不具备主动能力的成员保持被动监听但仍参与航迹覆盖。目标尚未进入区域时只执行区域覆盖扫描，不把区域外目标变成持续被动跟踪任务。目标进入区域并完成连续确认后，编组切换为 `PASSIVE_TRACK`，跟踪航迹限制在当前任务区域约束内。

人类专家可以将某个 UUV group 绑定到指定目标，形成 `DEDICATED_TRACK`。专属编组持续跟踪该目标，不因目标暂时离开普通区域而解除。任一专属 UUV 达到里程或能量边界时，进入 `RETURN_TO_REGION`，沿所属目标区域的扫描航迹返回；到达区域后清除专属锁定、里程归零并恢复普通的“多区域划分 + 区域轮转”模式。该专属资源轮转不应误生成母舰回收任务，除非后续普通任务轮转再次要求母舰回收。
...
四舰编队中只有三艘母舰进入投放/回收匹配池；航母只提供固定战斗群归属和主节点状态。母舰完成本次投放与回收航线后必须回归航母战斗群，进入待命/编队状态，并发布 `carrier_returned_to_fleet` 事件。

母舰一次任务可以连续访问多个区域；每次安装新的物理航线都会生成独立航次标识，返队事件按航次去重，连续出航不会被上一航次的事件抑制：
...
事件类型不能直接等同于战略重规划。所有观测、事件和人机交互都先进入持久化事件台账、记忆输入队列和证据链；只有确认会降低当前方案质量、效能或可行性的事件，才进入重规划控制路径。

### 8.1 四阶段事件管线

1. **完整采集**：周期性观测、`active_ping`、组报告、目标检测变化、UUV 生命周期事件、母舰事件、人类反馈和智能助理对话均保留原始来源 ID。实时降频不能删除这些来源。
2. **事件归一化**：按 `(scenario_id, entity_id, event_family)` 维护 episode，只有异常进入、异常升级或明确状态转换才产生控制事件；持续异常在恢复前不重复发射，恢复后释放 episode。
3. **方案影响评估**：将事件与当前执行方案、区域覆盖、时间窗、通信链路、UUV 资源、返航可行性和目标预测走廊比较。只有存在可验证影响时，才标记 `plan_impact=true`。
4. **批次凝练**：一个规划边界内只生成一个重规划批次，按严重程度合并多个来源事件，保留全部 `source_event_ids`、受影响目标/区域和影响指标。新方案没有实质变化时不递增版本。

### 8.2 关键事件判定

直接关键、但同一 episode 只触发一次：

~~~text
initialization
target_added / target_removed / target_lost / target_reacquired
major_failure / repair_infeasible
uuv_failed / uuv_capability_lost
uuv_range_exhausted / uuv_energy_depleted
...
target_intent_changed / imm_confidence_shifted
target_exit_predicted
~~~

其中质量、通信和协方差必须使用统一的持续时间、进入/恢复滞回和升级规则；意图和置信度必须经过连续分析确认，并证明当前区域、时间窗、接力或 UUV 编组发生实质影响。不能仅因数值小幅波动触发重规划。

以下事件默认只进入审计、记忆或任务状态路径，不触发 LLM 战略重规划：
...
llm_degraded
target_maneuver
target_detection_acquired / target_detection_lost
group_quality_warning
battery_rotation
active_ping / group_report_published / progress_report / state_changed
~~~

它们只有在方案影响评估确认当前任务质量、效能或可行性下降时，才升级为关键事件。`strategic_review` 是低频健康检查，不因定时到达而强制生成新方案；`llm_degraded` 只能记录降级并保留当前方案，不能把依赖 LLM 的重规划再次送入失败的 LLM 路径。

### 8.3 事件触发不变量

- 同一事件 episode 在恢复前最多产生一个控制触发；安全关键升级可以突破普通冷却。
- 质量、通信、协方差和资源阈值均采用状态边沿，不按每帧或固定冷却重复生成事件。
- 任务轮换由资源状态进入/退出和覆盖影响驱动；低电量预警可以触发确定性轮换，但不自动等同于 LLM 战略重规划。
- 事件批次中保留所有来源，实时帧只发布关键事件窗口；完整事件仍可从 SQLite、帧日志和 Memory Stream 回溯。

每次重规划使用不可变快照，包含 IMM/UIF 证据、概率栅格、区域执行状态、UUV 里程/能量/模式、母舰位置/库存/承诺航线、待处理关键事件、人工锁定关系和当前计划版本。
...
LLM 不可用时不伪造成功，沿用上一版已验证计划并记录降级事件；降级事件进入记忆和审计，但不触发新的 LLM 重规划循环。
...
LLM 的方案思考与记忆处理是两个独立过程。方案思考只在初始制定、关键事件动态调整或专家确认调整时产生可供操作员查看的说明；记忆处理由可停止的后台 worker 持续执行，不占用仿真推进锁，也不阻塞主任务执行。

### 11.2 短期和长期记忆

短期记忆按 `(user_id, scenario_id, conversation_id)` 隔离，接收关键事件、普通事件、周期性观测批次和人机交互消息，保存滚动摘要和有限条近期原始消息。消息数量、估算 token 或时间达到阈值时，后台 worker 必须调用真实 LLM 生成摘要，保留关键事实、偏好、目标、方案影响、来源 ID 和未完成事项；摘要版本递增，原始消息只保留有界窗口。实时事件过滤不能阻止记忆输入。
...
长期记忆只作为素材供给，永远不能直接替代短期记忆；最终方案推理和证据回答输入必须在短期上下文中组装。长期记忆版本不可变，新信息形成新版本并标记旧版本失效，保留 `memory_family_id`、版本号和 supersedes 关系。访问频次会提升权重，长期未访问的记忆会衰减；低于归档阈值的记忆由维护任务归档，用户可以显式删除。
...
结构化区域策略请求必须受输出预算约束：在 `max_tokens=4096` 的 LongCat 配置下，每次请求最多携带 4 个区域/候选区域，避免嵌套策略对象与模型 reasoning 内容把 JSON 截断。批次结果仍须逐批通过严格 schema、区域覆盖和资源校验后合并；最终执行物化再次执行每目标最多 4 个区域的硬上限。超时、截断或非法响应只能记录真实 LLM 错误并进入降级/重试路径，禁止用规则或静态策略冒充 LLM 结果。

## 12. 测试与完成定义

### 12.1 算法与领域测试

覆盖概率栅格坐标和稳定 ID、每目标最多 4 个执行区域、候选保留与未选原因、置信度尺寸关系、连续方形区域、时间窗和接力、主动到被动切换门限、区域外围投放/回收点、UUV 里程/能量、扫描和跟踪最低规模、未来储备拒绝、连续多站点路线、多槽位匈牙利、A* 区域禁行和强制返航、非法 LLM 输出、LLM 降级。

### 12.2 仿真集成测试

固定种子场景必须验证：固定 1 艘航母 + 3 艘母舰从配置位置启动；UUV 只归属母舰；母舰完成投放/回收后回归编队；同一母舰连续访问至少三个投放点；从航母战斗群出发并最终返回；路线不穿过区域；目标未进入前多 UUV 蛇形 ACTIVE_SCAN 并产生主动声呐事件；目标进入后 PASSIVE_TRACK；专属编组在里程不足时 RETURN_TO_REGION 并恢复普通模式；目标离开后交接与回收；最大里程触发轮换和重规划；意图/置信度变化只有在连续确认且影响当前方案时重新生成区域；正常投放/回收不触发战略 LLM；资源不足不全派；新帧无 USV 且包含完整四舰角色/关系；同 seed 与真实 LLM provider 产生可校验计划和路线。

### 12.3 API、回放和前端测试

覆盖新操作帧合约、旧帧兼容读取、新帧不发布 USV、概率栅格、黄色 0.66 区域、UUV 模式、母舰多站点路线、区域详情、事件顺序、计划版本增量、WebSocket 断连回收、实时方案丢帧后的快照补偿和桌面/窄屏 Playwright 验证。

第一阶段只有在以下条件全部满足时完成：UUV 完成主动扫描与被动跟踪切换；区域权严格交接；每个目标最终执行区域不超过 4 个且候选证据完整；里程事件触发回收和重规划；普通事件不触发战略 LLM；关键事件只在方案质量/效能/可行性受影响时触发；事件 episode 有进入/恢复滞回；所有事件、观测和人机交互进入记忆处理；LLM 只能使用短期上下文完成推理；投放按整体效能分批；编队固定为 1 航母 + 3 母舰且 UUV 后勤归属只落在母舰；目标未进入时执行多 UUV 蛇形主动覆盖，专属 UUV 里程不足后能回到区域并恢复普通模式；母舰可连续投放且有可用 UUV 时保持可匹配，任务完成后回归编队；每条母舰路线从航母战斗群出发并返回；A* 不穿越区域；匈牙利考虑距离、时间窗、容量和未来资源；地图显示完整舰队和角色；实时方案按计划版本增量同步；WebSocket 断连无未处理 ASGI 异常；旧回放兼容且新帧无 USV；后端、API、前端、真实 LLM 和本地 Embedding 长时测试通过。
