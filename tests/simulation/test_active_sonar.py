...
def test_active_echo_does_not_collapse_three_uuv_scan_route(tmp_path):
    config = load_app_config("configs/scenario/uuv_only_single_target.yaml")
    config = config.model_copy(
        update={
    ...
    for member_id in group.member_uuv_ids:
        engine._uuvs[member_id].position_xy = (-200.0, 20.0)
    engine._targets["target_00"].position_xy = target_xy

    engine._process_pings(start_s)

    commands = engine._waypoint_commands["target_00"]
    group_commands = tuple(commands[member_id] for member_id in group.member_uuv_ids)
    assert len(set(group_commands)) >= 2


def test_runtime_scan_coverage_reuses_unchanged_source_samples(tmp_path, monkeypatch) -> None:
    config = load_app_config("configs/scenario/uuv_only_single_target.yaml")
    controller = MissionController(scenario_id=config.scenario.scenario_id)
    engine = SimulationEngine(
        config,
    ...
    samples = [("active_ping:cached", 0.0, 0.0, 600.0)]
    engine._runtime_scan_ping_samples[group.group_instance_id] = samples
    engine._runtime_scan_ping_counts[group.group_instance_id] = 1

    original = engine_module.ping_footprint_coverage_fraction
    calls = 0
    ...
    monkeypatch.setattr(engine_module, "ping_footprint_coverage_fraction", count_calls)
    first = engine._runtime_scan_state_observations(mission)[region.region_id]
    second = engine._runtime_scan_state_observations(mission)[region.region_id]

    assert first == second
    assert calls == 1


def test_retired_runtime_uuv_entities_are_reclaimed(tmp_path) -> None:
    config = load_app_config("configs/scenario/uuv_only_single_target.yaml")
    controller = MissionController(scenario_id=config.scenario.scenario_id)
    engine = SimulationEngine(
        config,
    ...
    initial_uuv_ids = set(engine._uuvs)
    runtime_snapshot = _runtime_execution_snapshot()
    generated_group = runtime_snapshot.task_groups[0].model_copy(
        update={
            "member_uuv_ids": tuple(
                f"runtime-only:{index:02d}" for index in range(3)
            )
        }
    )
    engine._ensure_runtime_group_entities(generated_group)
    generated_uuv_ids = set(engine._uuvs) - initial_uuv_ids
    assert len(generated_uuv_ids) == 3
    engine._assignments["target_00"] = tuple(sorted(generated_uuv_ids))

    engine._reclaim_retired_runtime_uuvs(controller.snapshot())

    assert set(engine._uuvs) == initial_uuv_ids
    assert not generated_uuv_ids & set(engine._uuv_carrier_ids)
    assert "target_00" not in engine._assignments
