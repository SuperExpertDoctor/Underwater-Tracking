...
def epoch(*, public_target_prior_ids: tuple[str, ...] = ()) -> PlanningEpoch:
    return PlanningEpoch(
        ...
        public_target_prior_ids=public_target_prior_ids,
...
def public_group_report() -> GroupReport:
    return GroupReport(
        group_id="group-1",
        target_id="target-00",
        sim_time_s=100,
...
def test_deployed_resource_episode_is_rebased_for_a_slow_provider() -> None:
    planned_assignment = candidate().region_assignments[0].model_copy(
        update={"active_scan_uuv_ids": ("uuv-1",)}
    )
    planned = candidate().model_copy(
        update={
            "region_assignments": (planned_assignment,),
            "resource_episode_by_uuv": {"uuv-1": 0},
        }
    )
    deployed = mission().model_copy(
        update={
            "uuv_resources": {
                "uuv-1": UUVResourceState(
                    uuv_id="uuv-1",
    ...
    assert report.valid is True
    assert report.rebased_plan is not None
    assert report.rebased_plan.resource_episode_by_uuv == {"uuv-1": 1}


def test_onboard_resource_episode_is_rebased_after_a_completed_sortie() -> None:
    planned_assignment = candidate().region_assignments[0].model_copy(
        update={"active_scan_uuv_ids": ("uuv-1",)}
    )
    planned = candidate().model_copy(
...
def test_expired_prior_is_replaced_by_current_public_group_evidence() -> None:
    report = revalidate_executable_mission_plan(
        epoch=epoch(public_target_prior_ids=("prior-1",)),
        candidate=candidate(),
        current_situation=situation().model_copy(
            update={"group_reports": (public_group_report(),)}
        ),
        current_mission=mission(),
        current_expert_request_version=None,
        recovered_event_ids=frozenset(),
...
def test_changed_prior_without_current_public_evidence_invalidates_candidate() -> None:
    report = revalidate_executable_mission_plan(
        epoch=epoch(public_target_prior_ids=("prior-1",)),
        candidate=candidate(),
        current_situation=situation(),
    ...
    assert report.valid is False
    assert "prior_changed" in {issue.code for issue in report.issues}
