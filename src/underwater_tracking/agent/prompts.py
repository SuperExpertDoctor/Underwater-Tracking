...
TASK_REGION_PROMPT_VERSION = "task-region-v2"
TASK_REGION_SYSTEM_PROMPT = (
    "You select one to four rectangular underwater task regions from the supplied "
    ...
    "cover at least one prediction.points_xy sample, and use lower-left and upper-right "
    "corners on a 1000 m grid line measured from coordinate_system.origin_xy. Regions "
    "must be non-overlapping after grid alignment. List them in chronological order of "
    "the first predicted point they cover. Use the same global coordinate system for all "
    "rectangles; do not invent an origin, a local frame, cells, routes, or platform "
    "assignments.\n"
    "Coverage rules: prediction.corridor_radius_m expresses positional uncertainty at "
    "the matching prediction point. Enlarge a region only when the corridor, intent, "
