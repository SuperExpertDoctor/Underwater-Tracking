    ...
    *,
    ...
    if not isinstance(missing_entity_frames, Mapping):
        ...
        for entity_id, raw_missing_frames in missing_entity_frames.items():
            ...
            if (
                missing_frame_ids
                != tuple(sorted(set(missing_frame_ids)))
            ):
                valid = False
                continue
            present_frame_ids = set(range(first_frame_id, last_frame_id + 1)) - set(
                missing_frame_ids
            )
            if not present_frame_ids:
                valid = False
                continue
            first_present_frame_id = min(present_frame_ids)
            last_present_frame_id = max(present_frame_ids)
            lifecycle_bounded_missing = (
                *range(first_frame_id, first_present_frame_id),
                *range(last_present_frame_id + 1, last_frame_id + 1),
            )
            if missing_frame_ids != lifecycle_bounded_missing:
                valid = False
