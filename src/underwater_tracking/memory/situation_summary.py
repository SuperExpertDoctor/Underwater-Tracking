...
def build_periodic_situation_summary(
    ...
    targets = _target_summaries(situation, source_events)
...
def _uuv_counts(situation: SituationSnapshot, mission: MissionSnapshot) -> UUVCountSummary:
    ...
    for uuv_id in uuv_ids:
        ...
        deployment = (
            ...
            else _value(state.deployment_state) if state is not None else "unknown"
        )
...
def _target_summaries(
    situation: SituationSnapshot, source_events: Sequence[RuntimeEvent]
    ...
    adversaries = {summary.target_id: summary for summary in situation.adversary_summaries}
    target_ids = sorted(set(reports) | set(adversaries))
    results: list[PublicTargetSummary] = []
    for target_id in target_ids:
        target_report = reports.get(target_id)
        adversary = adversaries.get(target_id)
        ...
        if adversary is not None:
            inferred_intent = adversary.intent or inferred_intent
            intent_confidence = adversary.confidence or intent_confidence
    ...
    *,
    ...
    for region_current in regions:
        ...
        if (
            ...
            and previous_region.lifecycle != region_current.lifecycle
        ):
            changes.append(
                _change(
                    "region_lifecycle",
        ...
        if (
            previous_region is not None
            and previous_region.coverage != region_current.coverage
        ):
            changes.append(
                _change(
                    "region_coverage",
    ...
    if previous.uuv_counts.mode_counts != uuv_counts.mode_counts:
        changes.append(_change("uuv_mode", "fleet", _json_value(previous.uuv_counts.mode_counts), _json_value(uuv_counts.mode_counts)))
    if previous.uuv_counts.deployment_counts != uuv_counts.deployment_counts:
        changes.append(_change("uuv_deployment", "fleet", _json_value(previous.uuv_counts.deployment_counts), _json_value(uuv_counts.deployment_counts)))
    if (previous.uuv_counts.healthy, previous.uuv_counts.unhealthy) != (
        uuv_counts.healthy,
        uuv_counts.unhealthy,
    ):
        changes.append(_change("uuv_health", "fleet", previous.uuv_counts.healthy, uuv_counts.healthy))
    if previous.uuv_counts.energy_below_reserve_count != uuv_counts.energy_below_reserve_count:
        changes.append(_change("uuv_energy", "fleet", previous.uuv_counts.energy_below_reserve_count, uuv_counts.energy_below_reserve_count))
    if previous.uuv_counts.mileage_total_m != uuv_counts.mileage_total_m:
        changes.append(_change("uuv_mileage", "fleet", previous.uuv_counts.mileage_total_m, uuv_counts.mileage_total_m))
    ...
    for target_current in targets:
        ...
        if previous_target is None:
            continue
        if previous_target.quality_score != target_current.quality_score:
            changes.append(
                _change(
                    "target_quality",
...
def _change(change_type: str, entity_id: str, previous: _PublicValue, current: _PublicValue) -> SituationChange:
...
def _json_value(value: Mapping[str, int]) -> str:
    return json.dumps(dict(sorted(value.items())), sort_keys=True, separators=(",", ":"))
...
def _human_summary(summary: PeriodicSituationSummary) -> str:
    ...
    carriers = ",".join(
        f"{carrier.carrier_id}:{carrier.route_status}:onboard={carrier.onboard_uuv_count}:recoverable={carrier.recoverable_uuv_count}"
        for carrier in summary.carrier_states
    ) or "none"
    targets = ",".join(
        f"{target.target_id}:quality={target.quality_score:.2f}:intent={target.intent}:prediction={target.prediction_revision}"
    ...
    text = (
        ...
        f"changes={len(summary.changes_since_previous)}; sources={len(summary.source_event_ids)}"
    )
