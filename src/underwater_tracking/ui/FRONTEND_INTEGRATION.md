...
- 生产后端的语义检索由本地 `sentence-transformers` 模型提供；`configs/memory.yaml` 强制 `embedding_local_files_only: true`，模型权重需提前放入本地缓存或路径。LongCat 只用于聊天 LLM，不提供 embedding；包或权重缺失时 UI 必须显示真实 `degraded` 状态，不可用远程、哈希或 mock 向量替代。
