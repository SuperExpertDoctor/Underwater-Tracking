...
test.describe("main.py visual command center flow", () => {
  ...
  test("desktop exposes live adversarial state, three cards, human controls, and replay", async ({
    page,
  }) => {
    ...
    await expect(
      page.getByText(/目标侧估计|LLM 决策摘要/).first(),
    ).toBeVisible();
  ...
  test("narrow screen keeps live state readable and supports sidebar close", async ({
    page,
  }) => {
    const consoleErrors: string[] = [];
    page.on("console", (message) => {
    ...
    await expect(
      sidebar.getByText("目标潜艇脑", { exact: true }),
    ).toBeVisible();
    const scrolling = await sidebar.evaluate(
      (element) =>
