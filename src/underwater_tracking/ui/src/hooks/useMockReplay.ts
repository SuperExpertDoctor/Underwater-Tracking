...
import { getMockFrames, MOCK_FRAMES } from "../mocks/mockData";
...
export interface MockReplayMarker {
  frameIndex: number;
  timeS: number;
  type: string;
  label?: string;
...
export default function useMockReplay(enabled: boolean) {
  ...
  const loadRange = useCallback(async (startS = 0, endS?: number) => {
    const next = getMockFrames(startS, endS);
  ...
  return {
    files: ["mock://operational"],
    selectedFile: "mock://operational",
    load: loadRange,
    loadRange,
    frames,
    total: frames.length,
    ...
    error: "",
    markers,
    sourceFrameCount: MOCK_FRAMES.length,
  };
}
